...
<script type="text/javascript">
	(function(window,$){
		...
		var dekota={
			...
			getPerson:function(){
				$.ajax({
					url: dekota.url + '/service/person/101/',
					type: 'GET',
					dataType: 'json'
				}).done(function(data, status, xhr) {
					$.UIkit.notify("获取人员信息成功" + data, {status:'success',timeout:1000});
			...
			addPerson:function(){
				$.ajax({
					url: dekota.url + '/service/person',
					type: 'POST',
					dataType: 'json',
					data: {id: 1,name:'张三',sex:'男',age:23}
				}).done(function(data, status, xhr) {
					$.UIkit.notify("添加人员信息成功" + data, {status:'success',timeout:1000});
				}).fail(function(xhr, status, error) {
					$.UIkit.notify("请求失败！", {status:'danger',timeout:2000});
				});
			},
			delPerson:function(){
				$.ajax({
					url: dekota.url + '/service/person/109',
					type: 'DELETE',
					dataType: 'json'
				}).done(function(data, status, xhr) {
					$.UIkit.notify("删除人员信息成功" + data, {status:'success',timeout:1000});
				}).fail(function(xhr, status, error) {
					$.UIkit.notify("请求失败！", {status:'danger',timeout:2000});
				});
			},
			updatePerson:function(){
				$.ajax({
					...
					$.UIkit.notify("更新人员信息成功" + data, {status:'success',timeout:1000});
				}).fail(function(xhr, status, error) {
					$.UIkit.notify("请求失败！", {status:'danger',timeout:2000});
				});
			},
